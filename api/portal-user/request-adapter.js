...
const axiosInstance = axios.create({
  baseURL: process.env.NEXT_PUBLIC_RECYLINKER_API_URL + '/portal',
  timeout: 30000,
  headers: {
    common: {
...
axiosInstance.interceptors.request.use(async (config) => {
  const accessToken = Cookies.get('portalUserAccessToken')
  if (accessToken) {
  ...
  async (error) => {
    ...
    if (
      ...
      try {
        ...
        Cookies.set('portalUserRefreshToken', newRefreshToken, { expires: 30 })

        Cookies.set('portalUserAccessToken', data.access.token)
