...
export default function Header() {
  ...
  const { portalAgency } = useGetPortalAgency()
  const handlePortalLogout = async () => {
    const toastId = toast.loading('Please wait...')
    try {
      await portalLogOut({
        refreshToken: Cookies.get('portalAgencyRefreshToken') || '',
      })
        ...
        <NavbarItem>
          <Dropdown>
            ...
            <DropdownMenu>
              ...
              <DropdownItem key='2' startContent={<Power size={15} />}>
                Activate profile
              </DropdownItem>
              <DropdownItem
                key='3'
